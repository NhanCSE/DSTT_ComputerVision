...
| 10 | `np.cumsum(ratio)` / `np.searchsorted(cumvar, 0.95)` | Tính phương sai tích lũy và tìm `k` tối thiểu đạt 95% phương sai. | [recognizer.py:315,323](src/recognizer.py#L315) |
| 11 | `.reshape(112, 92)` | Chuyển vector 1D `(10304,)` → ảnh 2D `(112, 92)` để hiển thị. | [visualizer.py:40](src/visualizer.py#L40) |
| 12 | `.flatten()` | **Vector hóa ảnh**: chuyển ảnh 2D → vector 1D — bước đầu tiên trước khi áp dụng đại số tuyến tính. | [dataloader.py:189](src/dataloader.py#L189) |
| 13 | `np.mean(y_true == y_pred)` | Tính **accuracy** — tận dụng broadcasting và quy ước `True=1, False=0`. | [evaluator.py:37](src/evaluator.py#L37) |
...
| 7 | `st.image(arr, caption=...)` | Hiển thị ảnh kết quả (mean face, eigenfaces, ảnh nhận dạng). |
...
| 9 | $U^T U = I$ | `U.T @ U` (≈ $I$) | Kiểm tra hệ cơ sở **trực chuẩn** (orthonormal). |
